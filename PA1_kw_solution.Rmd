...
```
The average daily number of steps is `r dailymean`, and the median daily number of steps is `r dailymedian`.
...
There are `r Missing_total` missing values, or c.`r Missing_percent`% of total observations. Often observations are missing for entire days.  
...
```
The imputation increases the average and median daily steps to `r dailymean1` and `r dailymedian1`, respectively, and makes the histogram more symmetrical (because previously, several days had shown as zero steps due to missing values).  
